/* =========================================================
   Persian Music Scales — Audio Analyzer UI + integration v5.1 — quick scale finder + microphone capture
...
(() => {
  ...
  const elements = {
    ...
    quickFinderClose: $("#closeQuickScaleFinder"),
    quickRecordButton: $("#quickHoldToRecordButton"),
    ...
    quickUpload: $("#quickUploadTrackButton"),
    quickOpenAnalyzer: $("#quickOpenFullAnalyzer"),
  ...
  function openQuickFinder() {
    ...
    if (elements.drawer.classList.contains("open")) closeDrawer({ restoreFocus: false });
    elements.quickFinderPanel.hidden = false;
    elements.quickFinderButton.setAttribute("aria-expanded", "true");
    elements.quickFinder?.classList.add("panel-open");
    requestAnimationFrame(() => elements.quickFinderPanel.classList.add("open"));
    window.setTimeout(() => elements.quickRecordButton?.focus({ preventScroll: true }), 80);
  }

  function closeQuickFinder({ restoreFocus = true, cancelRecording = true } = {}) {
    if (!elements.quickFinderPanel || !elements.quickFinderButton) return;
    ...
    elements.quickFinderButton.setAttribute("aria-expanded", "false");
    elements.quickFinder?.classList.remove("panel-open");
    window.setTimeout(() => {
  ...
  function bindQuickFinderEvents() {
    if (!elements.quickFinderButton || !elements.quickFinderPanel) return;

    elements.quickFinderButton.addEventListener("click", () => {
      if (isQuickFinderOpen()) closeQuickFinder();
      else openQuickFinder();
    });
    elements.quickFinderClose?.addEventListener("click", () => closeQuickFinder());
    elements.quickUpload?.addEventListener("click", () => {
      closeQuickFinder({ restoreFocus: false, cancelRecording: false });
      elements.fileInput.click();
    });
    elements.quickOpenAnalyzer?.addEventListener("click", () => {
      closeQuickFinder({ restoreFocus: false, cancelRecording: false });
      openDrawer();
    });

    document.addEventListener("pointerdown", (event) => {
      if (!isQuickFinderOpen() || state.recorder.recording || state.recorder.starting) return;
      if (!elements.quickFinder.contains(event.target)) closeQuickFinder({ restoreFocus: false });
    });
  }
  ...
  async function finalizeMicrophoneRecording() {
    ...
    try {
      ...
      if (isQuickFinderOpen()) {
        window.setTimeout(() => closeQuickFinder({ restoreFocus: false, cancelRecording: false }), 520);
      }
      await loadFile(file);
      updateRecordingFeedback(t("recordReady"), 0, "ready");
    } finally {
  ...
  function setLanguage(language) {
    ...
    if (!state.selectedCandidate && elements.quickFinderHelp) {
      elements.quickFinderHelp.textContent = dictionary.quickFindScaleHelp;
    }
  ...
  function resetAnalysisResult(clearBadge = true) {
    ...
    if (clearBadge) {
      ...
      if (elements.quickFinderHelp) elements.quickFinderHelp.textContent = t("quickFindScaleHelp");
    }
  }
  ...
  function updateButtons() {
    ...
    recorderButtons().forEach((button) => {
      button.disabled = !state.recorder.supported || state.recorder.finalizing || (Boolean(state.worker) && !state.recorder.recording);
    });
  }
