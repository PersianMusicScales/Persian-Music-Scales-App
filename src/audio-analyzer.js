/* =========================================================
   Persian Music Scales — Audio Analyzer UI + integration v5 — microphone capture
...
(() => {
  ...
  function initializeMicrophoneRecorder() {
    ...
    if (!state.recorder.supported) {
      ...
      elements.recordButton.classList.add("unsupported");
  ...
  function bindMicrophoneRecorderEvents() {
    ...
    elements.recordButton.addEventListener("pointerdown", (event) => {
      ...
      event.preventDefault();
      elements.recordButton.setPointerCapture?.(event.pointerId);
      state.recorder.holdActive = true;
      beginMicrophoneRecording();
    });

    const releasePointer = (event) => {
      if (event?.pointerId != null && elements.recordButton.hasPointerCapture?.(event.pointerId)) {
        elements.recordButton.releasePointerCapture?.(event.pointerId);
      }
      state.recorder.holdActive = false;
      finishMicrophoneRecording();
    };
    elements.recordButton.addEventListener("pointerup", releasePointer);
    elements.recordButton.addEventListener("pointercancel", releasePointer);
    elements.recordButton.addEventListener("lostpointercapture", () => {
    ...
    elements.recordButton.addEventListener("blur", () => {
    ...
    elements.cancelRecording?.addEventListener("click", () => {
      cancelMicrophoneRecording({ announce: true });
    });
  ...
  async function beginMicrophoneRecording() {
    ...
    elements.recordButton.classList.add("arming");
    ...
    try {
      ...
      if (!recorder.holdActive || recorder.cancelled) {
        ...
        elements.recordButton.classList.remove("arming");
      ...
      elements.recordButton.setAttribute("aria-pressed", "true");
      if (elements.cancelRecording) elements.cancelRecording.hidden = false;
      ...
      elements.recordButton.classList.remove("arming", "recording");
      elements.recordButton.setAttribute("aria-pressed", "false");
  ...
  function finishMicrophoneRecording() {
    ...
    elements.recordButton.classList.add("processing");
    elements.recordButton.setAttribute("aria-pressed", "false");
  ...
  function cancelMicrophoneRecording({ announce = true } = {}) {
    ...
    recorder.starting = false;
    recorder.recording = false;
    elements.recordButton?.classList.remove("arming", "recording", "processing");
    elements.recordButton?.setAttribute("aria-pressed", "false");
    if (elements.cancelRecording) elements.cancelRecording.hidden = true;
  ...
  function startRecordingClock() {
    ...
    const update = () => {
      ...
      if (elements.recordingTimer) {
        elements.recordingTimer.textContent = formatRecordingTime(elapsed);
        elements.recordingTimer.setAttribute("datetime", `PT${elapsed.toFixed(1)}S`);
      }
    };
    update();
  ...
  async function startMicrophoneMeter(stream) {
    ...
    const draw = () => {
      ...
      if (elements.microphoneLevel) {
        elements.microphoneLevel.style.width = `${level}%`;
        elements.microphoneLevel.classList.toggle("clipping", level > 92);
      }
  ...
  function stopMicrophoneMeter() {
    ...
    if (elements.microphoneLevel) {
      ...
      elements.microphoneLevel.classList.remove("clipping");
    }
  }
  ...
  function updateRecordingFeedback(message, seconds = 0, mode = "ready") {
    if (elements.recordingStatus) elements.recordingStatus.textContent = message;
    if (elements.recordingFeedback) elements.recordingFeedback.dataset.state = mode;
    if (elements.recordingTimer && !state.recorder.recording) {
      elements.recordingTimer.textContent = seconds > 0 ? formatRecordingTime(seconds) : "0:00";
      elements.recordingTimer.setAttribute("datetime", `PT${Math.max(0, seconds).toFixed(1)}S`);
    }
  }
  ...
  function setLanguage(language) {
    ...
    if (!state.recorder.recording && !state.recorder.starting) {
      updateRecordingFeedback(state.recorder.supported ? dictionary.recordReady : dictionary.microphoneUnsupported, 0, state.recorder.supported ? "ready" : "error");
    }
  }
  ...
  function isPlaybackActive() {
    return (!elements.audio.paused && !elements.audio.ended) || state.melodyPlayer.playing;
  }
  ...
  function updateButtons() {
    ...
    if (elements.recordButton) {
      elements.recordButton.disabled = !state.recorder.supported || state.recorder.finalizing || (Boolean(state.worker) && !state.recorder.recording);
    }
  }
