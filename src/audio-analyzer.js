/* =========================================================
   Persian Music Scales — Audio Analyzer UI + integration v5.5
   iOS standalone playback fix + built-in example recording.
...
(() => {
  ...
  const EXAMPLE_TRACK = Object.freeze({
    url: "assets/audio/jane-oshagh.mp3",
    fileName: "Jane Oshagh.mp3",
    displayName: "Jane Oshagh",
  });
  ...
  const I18N = {
    en: {
      ...
      exampleDivider: "or try the built-in example",
      tryExample: "Try example recording",
      exampleTrackName: "Jane Oshagh",
      exampleHelp: "Load, analyze, and play the example directly from this application.",
      loadingExample: "Loading the example recording…",
      exampleFailed: "The example recording could not be loaded.",
      tapToHear: "Analysis is ready. Tap Original to hear the result.",
    ...
    fr: {
      ...
      exampleDivider: "ou essayer l’exemple intégré",
      tryExample: "Essayer l’enregistrement exemple",
      exampleTrackName: "Jane Oshagh",
      exampleHelp: "Chargez, analysez et écoutez l’exemple directement dans l’application.",
      loadingExample: "Chargement de l’enregistrement exemple…",
      exampleFailed: "L’enregistrement exemple n’a pas pu être chargé.",
      tapToHear: "L’analyse est prête. Touchez Original pour entendre le résultat.",
    ...
    fa: {
      ...
      exampleDivider: "یا نمونهٔ آماده را امتحان کنید",
      tryExample: "پخش و تحلیل نمونه",
      exampleTrackName: "Jane Oshagh",
      exampleHelp: "نمونه را مستقیماً از برنامه بارگذاری، تحلیل و پخش کنید.",
      loadingExample: "در حال بارگذاری نمونهٔ صوتی…",
      exampleFailed: "بارگذاری نمونهٔ صوتی ممکن نشد.",
      tapToHear: "تحلیل آماده است. برای شنیدن نتیجه، دکمهٔ قطعهٔ اصلی را لمس کنید.",
  ...
  const elements = {
    ...
    exampleTrack: $("#loadExampleTrackButton"),
  ...
  async function prepareMicrophonePermission() {
    ...
    try {
      /* Request microphone permission directly. On iOS, awaiting AudioContext
         setup before getUserMedia can leave the permission state stuck. */
  ...
  function bindQuickFinderEvents() {
    ...
    /* Recording/permission click behaviour is bound centrally in
       bindMicrophoneRecorderEvents(). This listener only suppresses navigation. */
  ...
  function bindEvents() {
    ...
    elements.dropZone.addEventListener("pointerdown", () => {
      primeOriginalAudioOutputFromGesture();
    }, { passive: true });
    elements.dropZone.addEventListener("click", () => {
      primeOriginalAudioOutputFromGesture();
      state.audioOutput.filePickerPending = true;
    ...
    elements.fileInput.addEventListener("change", () => {
      ...
      state.audioOutput.filePickerPending = false;
      primeOriginalAudioOutputFromGesture();
      if (file) loadFile(file);
    });
    elements.exampleTrack?.addEventListener("pointerdown", () => {
      primeOriginalAudioOutputFromGesture();
    }, { passive: true });
    elements.exampleTrack?.addEventListener("click", () => {
      primeOriginalAudioOutputFromGesture();
      loadExampleTrack();
    });
    ...
    window.addEventListener("pageshow", () => {
      resumeOriginalAudioOutputIfPossible();
    });
    document.addEventListener("visibilitychange", () => {
      if (!document.hidden) resumeOriginalAudioOutputIfPossible();
    });
  ...
  function bindMicrophoneRecorderEvents() {
    const bindRecordButton = (button) => {
      ...
      button.addEventListener("pointerdown", (event) => {
        ...
        /* Before permission is ready, allow this gesture to complete as a
           normal tap. The click handler below performs the one-time setup.
           This prevents the iOS permission dialog from breaking a hold. */
      ...
      button.addEventListener("click", (event) => {
        event.preventDefault();
        if (microphonePermissionIsReady() || state.recorder.permissionPreparing) return;
        state.recorder.holdActive = false;
        state.recorder.ignoreRelease = true;
  ...
  async function loadExampleTrack() {
    if (state.exampleLoading || state.worker || state.recorder.recording || state.recorder.starting) return;
    state.exampleLoading = true;
    if (elements.exampleTrack) {
      elements.exampleTrack.disabled = true;
      elements.exampleTrack.setAttribute("aria-busy", "true");
    }
    setStatus(t("loadingExample"), 3);
    updateButtons();

    try {
      const response = await fetch(new URL(EXAMPLE_TRACK.url, document.baseURI), {
        cache: "default",
        credentials: "same-origin",
      });
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      const blob = await response.blob();
      const file = fileFromBlob(blob, EXAMPLE_TRACK.fileName);
      await loadFile(file, { origin: "example" });
    } catch (error) {
      console.error("Example recording could not be loaded:", error);
      setStatus(t("exampleFailed"), 0, true);
    } finally {
      state.exampleLoading = false;
      if (elements.exampleTrack) {
        elements.exampleTrack.disabled = false;
        elements.exampleTrack.removeAttribute("aria-busy");
      }
      updateButtons();
    }
  }
  ...
  async function loadFile(file, { origin = "file" } = {}) {
    ...
    state.file = null;
    state.inputOrigin = origin === "microphone"
      ? "microphone"
      : origin === "example"
        ? "example"
        : "file";
  ...
  async function startAutomaticPlaybackAndClose() {
    ...
    try {
      await resumeOriginalAudioOutputIfPossible();
      started = await startBufferPlayback(0, {
        loop: state.inputOrigin === "microphone",
        automatic: state.inputOrigin === "microphone",
      });
    } catch (error) {
    ...
    if (!started) {
      state.audioOutput.needsUserTap = true;
      const message = (IS_IOS && IS_STANDALONE) ? t("tapToHear") : t("autoplayBlocked");
      setStatus(message, 100);
      if (state.inputOrigin === "microphone") {
        updateRecordingFeedback(message, 0, "error");
      }
      elements.playPause.textContent = t("play");
      elements.playPause.classList.remove("playing");
      elements.playPause.classList.add("attention");
      elements.playPause.focus({ preventScroll: true });
      return;
    }

    elements.playPause.classList.remove("attention");
    if (state.inputOrigin === "microphone") {
  ...
  function resetPlaybackForNewTrack() {
    ...
    state.playbackKind = "none";
    state.cursorTime = 0;
    state.audioOutput.needsUserTap = false;
    elements.playPause.textContent = t("play");
    elements.playPause.classList.remove("playing", "attention");
    if (elements.melodyPlayPause) {
  ...
  async function toggleOriginalPlayback() {
    if (!state.file || !state.analysis || !state.audioBuffer) return;
    try {
      primeOriginalAudioOutputFromGesture();
      elements.playPause.classList.remove("attention");
      ...
      const started = await startBufferPlayback(state.cursorTime, {
        loop: state.inputOrigin === "microphone",
        automatic: false,
      });
      if (!started) {
        state.audioOutput.needsUserTap = true;
        setStatus(t("tapToHear"), 100);
        elements.playPause.classList.add("attention");
      }
    } catch (error) {
      console.error(error);
  ...
  function seekFromSlider() {
    ...
    const bufferLoop = state.bufferPlayer.loop;
    ...
    if (bufferWasPlaying) {
      startBufferPlayback(targetTime, { loop: bufferLoop, automatic: bufferAutomatic }).catch(console.error);
    }
  ...
  function createOriginalVisualizerGraph() {
    if (state.visualizer.originalContext) return state.visualizer.originalContext;
    const AudioContextClass = window.AudioContext || window.webkitAudioContext;
    ...
    state.visualizer.originalSource = null;
  ...
  function playAudioUnlockPulse(context) {
    ...
    try {
      ...
      console.debug("Audio unlock pulse was not available:", error);
    }
  }

  function primeOriginalAudioOutputFromGesture() {
    const context = createOriginalVisualizerGraph();
    if (!context) return Promise.resolve(false);

    state.audioOutput.lastPrimeAt = Date.now();
    ...
    state.audioOutput.primePromise = Promise.resolve(resumePromise)
      .then(() => {
        playAudioUnlockPulse(context);
        state.audioOutput.primed = context.state === "running";
        state.audioOutput.needsUserTap = !state.audioOutput.primed;
        return state.audioOutput.primed;
      })
      .catch((error) => {
        console.info("Audio output could not be primed yet:", error);
        state.audioOutput.needsUserTap = true;
        return false;
      });

    return state.audioOutput.primePromise;
  }

  async function resumeOriginalAudioOutputIfPossible() {
    const context = state.visualizer.originalContext;
    ...
    if (context.state !== "running") {
      try {
        ...
        console.info("Audio output remains locked:", error);
      }
    }
    state.audioOutput.primed = context.state === "running";
    state.audioOutput.needsUserTap = !state.audioOutput.primed;
    if (state.audioOutput.primed) playAudioUnlockPulse(context);
    return state.audioOutput.primed;
  }
  ...
  async function ensureOriginalVisualizerGraph() {
    ...
    if (state.audioOutput.primePromise) {
      try { await state.audioOutput.primePromise; } catch (_) {}
    }
    if (context.state !== "running") {
      try { await context.resume(); } catch (_) {}
    }
