...
(() => {
  ...
  function renderRadialSpectrum(noteEvent) {
    ...
    for (let index = 0; index < barCount; index += 1) {
      ...
      const pitchClass = mod(Math.floor((index / barCount) * 24), 24);
      const pitchValue = pitchEnergy[pitchClass];
      const melodyDistance = activePitch == null ? 12 : circularDistance(pitchClass, activePitch, 24);
      const melodyFocus = activePitch == null ? 0 : Math.exp(-0.5 * Math.pow(melodyDistance / 1.25, 2));

      let target;
      if (mode === "spectrum") target = spectrumValue;
      else if (mode === "pitch") target = pitchValue * 0.92 + spectrumValue * 0.08;
      else if (mode === "melody") target = pitchValue * 0.28 + melodyFocus * melodyEmphasis;
      else if (mode === "minimal") target = melodyFocus * melodyEmphasis * 0.88 + pitchValue * 0.12;
      else target = spectrumValue * 0.34 + pitchValue * 0.48 + melodyFocus * melodyEmphasis * 0.36;
  ...
  function drawRadialBars(values, noteEvent) {
    ...
    values.forEach((value, index) => {
      ...
      const pitchClass = mod(Math.floor((index / values.length) * 24), 24);
      const activeDistance = activePitch == null ? 12 : circularDistance(pitchClass, activePitch, 24);
      const activeStrength = activePitch == null ? 0 : Math.exp(-0.5 * Math.pow(activeDistance / 1.2, 2));
